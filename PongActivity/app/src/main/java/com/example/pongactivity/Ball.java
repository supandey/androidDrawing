...
public class Ball {
    ...
    Ball(int screenX) {
        ...
        // We do it here because we only want to do it once. WE will initialize the detail at the start of each game
        mRect = new RectF();
    }
    ...
    void update(long fps) {
        // Move the ball based upon the horizontal (mXVelocity) and vertical(mYVelocity) speed and the current frame rate(fps)
    ...
    // Bounce the ball back based upon whether it hits the left or right hand side
    void batBounce(RectF batPosition){
        ...
        // Having calculated left or right for horizontal direction simply reverse the vertical direction to go back up the screen
        reverseYVelocity();
    }
