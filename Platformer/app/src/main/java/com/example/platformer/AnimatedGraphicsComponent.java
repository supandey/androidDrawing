...
public class AnimatedGraphicsComponent implements GraphicsComponent {
    ...
    public void initialize(Context context, GameObjectSpec spec, PointF objectSize, int pixelsPerMetre) {
        ...
        mAnimator = new Animator(objectSize.y, objectSize.x, spec.getNumFrames(), pixelsPerMetre);

        // stretch the bitmap by the number of frames
        float totalWidth = objectSize.x * spec.getNumFrames();

        mBitmapName = spec.getBitmapName();
        BitmapStore.addBitmap(context, mBitmapName, new PointF(totalWidth, objectSize.y), pixelsPerMetre, true);
    ...
    public void draw(Canvas canvas, Paint paint, Transform t, Camera cam) {
        ...
        // Get the section of bitmap to draw when an object is in motion OR if it is a object with zero speed(like a fire tile)
