...
public class PhysicsEngine {
    ...
    private void detectCollisions(GameState gs, ArrayList<GameObject> objects) {
        ...
        // Something collides with some part of the player most frames so lets make some handy references
        // Get a reference to the players position as we will probably need to update it
        ...
        // Get the players extra colliders from the cast object
        ...
        for (GameObject go : objects) {
            ...
            if (go.checkActive()) {
                ...
                if (RectF.intersects(go.getTransform().getCollider(), playersTransform.getCollider())) {
                    ...
                    // A collision of some kind has occurred so lets dig a little deeper
                    collisionOccurred = true;
                    // Get a reference to the current (being tested) object's transform and location
                    ...
                    if (objects.indexOf(go) != LevelManager.PLAYER_INDEX) {
                        ...
                        for (int i = 0; i < playerColliders.size(); i++) {
                            if (RectF.intersects(testedTransform.getCollider(), playerColliders.get(i))) {
                                ...
                                // React to the collision based on the body part and object type
                                switch (go.getTag() + " with " + "" + i) {
                                    // Test feet first to avoid the player sinking in to a tile and unnecessarily triggering right and left as well
                                    case "Movable Platform with 0":// Feet
                                        playersPlayerTransform.grounded();
                                        playersLocation.y = (testedTransform.getLocation().y) - (playersTransform.getSize().y);
                                        break;
                                    ...
                                    case "Inert Tile with 2":// Right
                                        ...
                                        playersLocation.x = (testedTransform.getLocation().x) - playersTransform.getSize().x;
                                        break;
                                    ...
                                    case "Inert Tile with 3":// Left
                                        ...
                                        playersLocation.x = testedLocation.x + testedTransform.getSize().x;
                                        break;
