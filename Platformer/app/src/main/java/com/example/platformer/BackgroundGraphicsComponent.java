...
public class BackgroundGraphicsComponent implements GraphicsComponent {
    ...
    @Override
    public void initialize(Context context, GameObjectSpec spec, PointF objectSize, int pixelsPerMetre) {

        mBitmapName = spec.getBitmapName();
        BitmapStore.addBitmap(context, mBitmapName, objectSize, pixelsPerMetre,true);
    }
    ...
    public void draw(Canvas canvas, Paint paint, Transform t, Camera cam) {
        ...
        Bitmap bitmapRevesed = BitmapStore.getBitmapReversed(mBitmapName);
        ...
        float floatstartY = ((cam.getyCentre() - ((cam.getCameraWorldCentreY() - position.y) * cam.getPixelsPerMetreY())));
        int startY = (int) floatstartY;
        float floatendY = ((cam.getyCentre() - ((cam.getCameraWorldCentreY() - position.y - t.getSize().y) * cam.getPixelsPerMetreY())));
